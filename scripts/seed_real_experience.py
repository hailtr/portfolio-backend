        ...
        "translations": {
            "es": {
                ...
                "description": (
                    ...
                    "- Diseñé y ejecuté soluciones de scraping contra las plataformas más protegidas del mundo: Ticketmaster (una de las apps más difíciles de scrapear — scraper local + RabbitMQ + S3Queue → ClickHouse), TikTok (curl_cffi + fingerprinting móvil + rotación residencial, 72% success vs WAF/SlardarWAF), Genius (circuit breakers + per-token budgets vs rate limits), Tunefind (API + website dual-source con throttle adaptativo), Shazam y Discogs — sin muro que no haya escalado\n"
            ...
            "en": {
                ...
                "description": (
                    ...
                    "- Designed and executed scraping solutions against the most heavily protected platforms in the world: Ticketmaster (one of the hardest apps to scrape — local scraper + RabbitMQ + S3Queue → ClickHouse), TikTok (curl_cffi + mobile fingerprinting + residential rotation, 72% success vs WAF/SlardarWAF), Genius (circuit breakers + per-token budgets vs rate limits), Tunefind (API + website dual-source with adaptive throttling), Shazam, and Discogs — no wall I couldn't climb\n"
