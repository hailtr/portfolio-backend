...
def main():
    ...
    success, data = test_endpoint(
        "Get All Entities (ES)", f"{BASE_URL}/entities?lang=es"
    )
    all_passed = all_passed and success
    if success and data:
        ...
        for entity in data:
            print(
                f"     - {entity.get('slug')} ({entity.get('type')}) - {entity.get('title')}"
            )

    # Test get all entities (EN)
    success, data = test_endpoint(
        "Get All Entities (EN)", f"{BASE_URL}/entities?lang=en"
    )
    all_passed = all_passed and success

    # Test get single entity (if we have data)
    success, entities = test_endpoint(
        "Get Entities for slug test", f"{BASE_URL}/entities?lang=es"
    )

    if success and entities and len(entities) > 0:
        slug = entities[0].get("slug")
        success, data = test_endpoint(
            f"Get Entity by Slug ({slug})", f"{BASE_URL}/entities/{slug}?lang=es"
        )
        all_passed = all_passed and success
    else:
    ...
    success, data = test_endpoint("Get Available Languages", f"{BASE_URL}/languages")
    all_passed = all_passed and success
    if success and data:
        print(f"   Available: {data.get('languages', [])}")

    # Test categories endpoint
    success, data = test_endpoint("Get Categories", f"{BASE_URL}/categories")
    all_passed = all_passed and success

    # Test tags endpoint
    success, data = test_endpoint("Get Tags", f"{BASE_URL}/tags")
    all_passed = all_passed and success
