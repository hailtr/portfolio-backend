...
def test_health_check():
    ...
    try:
        ...
        if services.get("cache_type") == "redis":
            ...
            print_info(
                "For production, consider adding Redis via REDIS_URL env variable"
            )
...
def test_compression():
    ...
    print_success(
        f"Reduction: {reduction:.1f}% ({size_uncompressed - size_compressed:,} bytes saved)"
    )
...
def main():
    ...
    print(f"\n{Fore.MAGENTA}{'='*60}")
    print(
        f"{Fore.GREEN}✓ All systems operational!"
        if failed == 0
        else f"{Fore.YELLOW}⚠ Some tests failed"
    )
