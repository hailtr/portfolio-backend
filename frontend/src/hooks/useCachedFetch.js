    ...
    const fetchData = async () => {
      ...
      if (cacheKey) {
        ...
        if (cachedItem) {
          try {
            ...
            log(`[Cache Hit] Showing stale data for: ${cacheKey}`);
            setData(parsed.data);
      ...
      try {
        ...
        if (JSON.stringify(jsonData) !== JSON.stringify(cachedData)) {
          log(`[Cache Update] Data refreshed for: ${cacheKey}`);
          if (cacheKey) {
