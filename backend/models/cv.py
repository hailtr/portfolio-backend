...
class CVProfile(db.Model):
    ...
    # Relationships
    translations = db.relationship("CVTranslation", backref="profile", cascade="all, delete-orphan")
    sections = db.relationship("CVSection", backref="profile", cascade="all, delete-orphan")
    ...
    def to_dict(self, lang='es'):
        ...
        education_sections = [s.to_dict() for s in self.sections if s.section_type == 'education']
        skill_sections = [s.to_dict() for s in self.sections if s.section_type == 'skill']
...
class CVTranslation(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('profile_id', 'lang', name='_cv_profile_lang_uc'),)
...
class CVSection(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    profile_id = db.Column(db.Integer, db.ForeignKey('cv_profiles.id'), nullable=False)
    section_type = db.Column(db.String(32), nullable=False)  # 'work', 'education', 'skill', 'language', 'interest', 'award'
