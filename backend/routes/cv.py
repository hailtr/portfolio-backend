...
def build_cv_from_entities(lang="es"):
    ...
    try:
        ...
        if profile_entity:
            ...
            profile_trans = next(
                (t for t in profile_entity.translations if t.lang == lang), None
            )
            if profile_trans:
        ...
        for entity in work_entities:
            ...
            work_item = {
                ...
                "highlights": (
                    translation.content.get("highlights", [])
        ...
        for entity in skill_entities:
            ...
            skill_display = (
                f"{translation.title} ({level_display})"
                if level_display
        ...
        category_names = {
            "languages": (
                "Lenguajes de Programación" if lang == "es" else "Programming Languages"
            ),
            "data-engineering": (
                "Ingeniería de Datos" if lang == "es" else "Data Engineering"
            ),
            ...
            "visualization": (
                "Visualización de Datos" if lang == "es" else "Data Visualization"
            ),
        ...
        for entity in edu_entities:
            ...
            edu_item = {
                ...
                "startDate": entity.meta.get("startDate", ""),
                "endDate": entity.meta.get(
                    "endDate", "Actualidad" if lang == "es" else "Present"
                ),
        ...
        has_content = (
            cv_data["basics"]["name"]
            and cv_data["basics"]["name"] != ""
            and cv_data["basics"]["summary"]
            and len(cv_data["skills"]) > 0
        )
...
def cv_pdf():
    ...
    try:
        ...
        return (
            jsonify({"error": "PDF generation not available", "message": str(e)}),
            503,
        )
    except Exception as e:
